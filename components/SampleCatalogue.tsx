            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[#0d1117]">
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {SAMPLE_PROJECTS.map((sample) => (
                     ...
                     >
                        {/* Visual Header - Compacted or Image */}
                        <div className={`h-28 bg-gradient-to-br ${sample.color} relative overflow-hidden`}>
                           {sample.image ? (
                              <>
                                 <img src={sample.image} alt={sample.name} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" />
                                 <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent"></div>
                                 <div className="absolute bottom-2 left-2 text-white drop-shadow-md">
                                    {React.cloneElement(sample.icon as React.ReactElement<any>, { size: 20 })}
                                 </div>
                              </>
                           ) : (
                           ...
                           <div className="absolute top-2 right-2 text-[9px] font-mono bg-black/50 text-white px-1.5 py-0.5 rounded backdrop-blur-sm border border-white/10">
