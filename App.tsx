...
import { Code, MonitorPlay, Settings as SettingsIcon, ZoomIn, ZoomOut, RotateCcw, RotateCw, FileJson, CircleHelp, BookOpen, Edit } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleWidgetEvent = (eventName: string, widgetId: string) => {
    ...
    const evt = widget.events?.find(e => e.trigger === eventName);
    if (!evt) return;

    if (evt.action === 'NAVIGATE' && evt.targetScreenId) {
      handleSetActiveScreen(evt.targetScreenId);
    }
  };
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <header className="h-14 bg-slate-900 border-b border-slate-700 flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all ${isPreview
              ? 'bg-green-600/20 border-green-500/50 text-green-400'
              : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-slate-200'
              }`}
          >
            {isPreview ? <MonitorPlay size={18} /> : <Edit size={18} />}
            <span className="text-xs font-medium">{isPreview ? 'Preview Active' : 'Edit Mode'}</span>
          </button>
          ...
          <button onClick={() => setShowReference(true)} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-md text-sm font-medium transition-colors border border-slate-700"><BookOpen size={16} className="text-emerald-400" /> Examples</button>
      ...
      <ReferenceViewer isOpen={showReference} onClose={() => setShowReference(false)} />
