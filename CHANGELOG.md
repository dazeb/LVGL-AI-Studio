...
- **Preview Mode** ▶️: Toggle between "Edit" and "Preview" modes. Preview mode enables interactive widget testing (Buttons, Switches) and verifies "Navigate" event logic directly in the browser.
- **Canvas Zoom** 🔍: Added `Alt + Scroll` hotkey to zoom the canvas (20% - 300%).
- **Help System** ❓: Added a Help Dialog (top-right header) listing keyboard shortcuts and documentation key points.
- **New Templates**: Added "Setup Wizard", "Smart Home Hub", and "Music Player" to the template library.
- **Icons**: Expanded the internal icon set with standard playback and navigation symbols (Lucide React).

### Changed

- **Arc Widget**: Updated default styling to use a transparent container background with an distinct track color, replacing the previous gray box look.
- **Zoom Control**: Changed Zoom hotkey from `Ctrl + Scroll` to `Alt + Scroll` to prevent browser conflicts.

### Fixed

- **Preview Interactions**: Fixed an issue where click events were not propagating correctly in Preview Mode.
- **Build System**: Resolved missing `@types/node` TypeScript errors.
